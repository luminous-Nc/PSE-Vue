...
export const usePLCInterfacingStore = defineStore('PLCInterfacing', {
    ...
    actions: {
        async loadPLCCase(plcCaseName) {
            if (!this.current_case_study) {
                console.log(`Downloading PLC interfacing cases from ${apiBaseUrl}/assets/database/PLC_interfacing.json`)
                try {
                    const response = await fetch(`${apiBaseUrl}/assets/database/PLC_interfacing.json`);
                    const data = await response.json();
                    let targetCase = data.case_study.find(oneCase => oneCase.case_path === plcCaseName);
                    if (targetCase) {
                        ...
                        console.error(`Case study with ID ${plcCaseName} not found.`);
                    }
                } catch (error) {
