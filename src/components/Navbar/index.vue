        ...
        >
          <li>
            <router-link
              ...
              class="block py-2 px-3 rounded border-0 hover:text-blue-700 p-0"
              >Learn</router-link
            >
          </li>
          <li>
            <router-link
              to="/learningStyle"
              :class="{
                'text-blue-700': isActive('/learningStyle'),
                'text-gray-900': !isActive('/learningStyle'),
              }"
              class="block py-2 px-3 rounded border-0 hover:text-blue-700 p-0"
              >{{ LearningStyle }}</router-link
            >
          </li>
        </ul>
      </div>
...
const showQuestionnaire = computed(() => {
  if (
    studentStore.learningStyle === "null" ||
    studentStore.learningStyle === null
  ) {
    return true;
  } else {
...
const LearningStyle = computed(() => {
  return studentStore.learningStyle;
});
</script>
