<template>
  <div>
    <div v-show="!ifFirstStep"
         class="absolute left-10 bottom-10 m-4 w-20 h-20 bg-green-500 rounded-full flex items-center justify-center group hover:bg-green-600"
         @click="lastButton">
      <div
          class="w-0 h-0 border-t-[24px] border-r-[36px] border-b-[24px] transform -translate-x-1 border-transparent border-r-black group-hover:border-r-white"></div>
    </div>

    <div v-show="showNextStep"
         class="absolute right-10 bottom-10 m-4 w-20 h-20 bg-green-500 rounded-full flex items-center justify-center group hover:bg-green-600"
         @click="nextButton">
      <div
          class="w-0 h-0 border-t-[24px] border-l-[36px] border-b-[24px] transform translate-x-1 border-transparent border-l-black transform group-hover:border-l-white"></div>
    </div>

    <div v-show="showFunctionButton"
         class="absolute right-10 bottom-32 m-4 w-20 h-20 bg-green-500 rounded-full flex items-center justify-center group hover:bg-green-600"
         @click="functionButton">
      {{ caseStudyStore.function_key_name }}
    </div>

    <div v-show="showResetButton"
         class="absolute right-10 bottom-[13.5rem] m-4 w-20 h-20 bg-green-500 rounded-full flex items-center justify-center group hover:bg-green-600"
         @click="resetButton">
      reset
    </div>

    <div v-show="showReplayButton"
         class="absolute right-10 bottom-[13.5rem] m-4 w-20 h-20 bg-green-500 rounded-full flex items-center justify-center group hover:bg-green-600"
         @click="replayButton">
      replay
    </div>

    <div v-show="showWireSteps" class="absolute text-xl right-[4.8rem] bottom-16 group hover:text-green-700"
         @click="nextButton">
      skip
    </div>

    <div v-show="showWireSteps" class="absolute text-lg right-[4.8rem] bottom-24"> {{ caseStudyStore.current_wire }} /
      {{ caseStudyStore.total_wire }}
    </div>
  </div>
</template>
...
function functionButton() {
  if (caseStudyStore.function_key_name === 'start') {
    ...
    caseStudyStore.function_key_name = 'resubmit'
    caseStudyStore.allow_next_step = true
  } else if (caseStudyStore.function_key_name === 'resubmit') {
    let MyAnalysis = Init_Analysis();
    caseStudyStore.allow_next_step = true
  }
}
...
const showWireSteps = computed(() => {
  if (currentStep.value.type === "schematic") {
    if (caseStudyStore.show_function_button === false) {
      return false
    }
