<template>
  ...
  <div
    ...
    <div class="flex-shrink-0 basis-1/3 overflow-y-auto text-lg flex-col flex w-full">
    ...
    <h3 class=" flex-shrink-0 basis-1/5 mb-2 text-2xl text-gray-900 dark:text-white mt-3">
      {{ studentStore.getCurrentStep?.text }}</h3>
    ...
    <div class="justify-center flex mb-3 flex-shrink-0 basis-1/12 text-2xl">
      <button v-show="studentStore.currentStepFinished && studentStore.currentStep.type!=='finish'"
              @click="nextButton" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        Continue
      </button>

      <button v-show="studentStore.currentStep?.type==='finish'"
              @click="resetLearning" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        Reset
      </button>
...
watch(() => studentStore.currentStep, (newStep) => {
